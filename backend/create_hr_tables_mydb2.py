...
if __name__ == '__main__':
    try:
        ...
        seed_data(cur)
        verify(cur)
        print("✅ HR tables ready in mydb2. Try: SELECT * FROM departments; SELECT e.name, COUNT(p.id) FROM employees e JOIN projects p ON p.employee_id=e.id WHERE p.end_date IS NULL GROUP BY e.id HAVING COUNT(p.id)>1;")
    except Error as e:
        print("❌ MySQL error:", e)
    finally:
        try:
